...
- El HTML completo de la infografía (no una imagen de referencia) — normalmente dejado como archivo en `HTML/<archivo>.html` en la raíz del repo (carpeta de intake); si no aparece ahí, buscarlo o preguntar dónde quedó.
- El objeto JSON de metadata ya redactado (mismo schema de INFOGRAFIA-INVESTIGAR.md), pegado directamente en el chat.

En este flujo **no se investiga ni se redacta nada de nuevo** — ese trabajo ya vino resuelto. La labor se reduce a validar, organizar, renderizar y publicar:
...
3. **Validar la paleta de colores contra el sistema de diseño del proyecto** antes de guardar nada: revisar las variables de color del HTML entregado contra la sección "Paleta de Colores" de INFOGRAFIA-SPEC.md (verde predominante + un acento tierra, sin colores fuera de esa paleta salvo blanco/gris/negro) y contra los tonos ya usados en piezas existentes del mismo tema (para mantener continuidad visual dentro del tema). Si el HTML llega con una paleta distinta (ej. un tema oscuro azul/púrpura, como pasó antes con un diseño externo) y no es una pieza de evento (única excepción que permite paleta libre, ver "Branding de Evento"), ajustar las variables de color al verde+tierra del proyecto antes de exportar — no publicar la pieza fuera de paleta tal cual llegó.
4. **Validar que el HTML no invente cifras/métricas** (misma regla de "Reglas Importantes" e INFOGRAFIA-INVESTIGAR.md que aplica siempre): si trae porcentajes, barras o benchmarks numéricos específicos que no son datos reales verificables (frecuente en comparativas de modelos/herramientas), convertirlos a niveles cualitativos (Bajo/Medio/Alto, etc.) sin el falso porcentaje de precisión, manteniendo el mismo orden relativo que ya planteaba el HTML original — no publicar métricas inventadas tal cual llegaron.
5. Guardar el HTML (ya validado/ajustado si hizo falta) como `Construidos/<Tema>/<topico-slug>.html`.
6. Renderizarlo y exportarlo a PNG con el mismo pipeline técnico del flujo normal (Chrome headless 2x, recorte del `.canvas` con PIL — ver paso 4 de "Flujo para crear una infografía nueva") — verificando con medición real (idealmente ejecutando el mismo binario de Chrome headless con un script inyectado que mida `getBoundingClientRect`, no la vista previa del navegador del entorno de trabajo, que puede rendear con un viewport angosto y dar medidas poco confiables) que ningún elemento desborda el canvas de 1200×627 antes de darlo por bueno; si desborda, corregirlo antes de exportar.
7. **No generar `.prompt.md`** para esta pieza — no hay contexto de diseño propio ni investigación que documentar en este flujo. Solo generarlo si el usuario lo pide explícitamente.
8. Agregar el objeto JSON recibido **tal cual** al final de `publicaciones.json` (sin reescribir el contenido/redacción que ya viene resuelto), pero sí corrigiendo formato/valores que no cumplan el schema cerrado antes de guardar — casos frecuentes: `categoriaPrincipal`/`temas`/`tipoContenido` como etiqueta legible ("Inteligencia Artificial", "Infografía Técnica") en vez de la clave de `taxonomia.json` ("inteligencia-artificial", "infografia"); `temas` con valores que no existen en la taxonomía (mapear a los más cercanos ya existentes, o preguntar si se prefiere proponer uno nuevo); `imagen` sin el prefijo `Construidos/<Tema>/`; `fechaPublicacion` que no sigue la regla de +2 días sobre la última entrada del catálogo (salvo pieza de evento); `es`/`en`.descripcion fuera del rango de 180–250 palabras (expandir o recortar manteniendo el mismo contenido, sin inventar datos nuevos). Confirmar con el usuario los mapeos ambiguos antes de decidir solo.
9. Publicar como siempre (ver paso 6 del flujo normal): copiar el PNG a `paginaweb/publications/` con nombre `Title_Case_Con_Guion_Bajo`, y agregar la misma entrada a `paginaweb/publications/publicaciones.js` con su propio `imagen: "publications/..."` y su propia fecha (+2 días sobre la última entrada de ese archivo, salvo que el JSON entregado ya indique que es una pieza de evento).
10. Si la pieza resulta ser una actualización de una publicación existente (mismo tópico exacto), aplica igual la regla de consolidación de "Gestión inteligente de publicaciones e infografías" (archivar versión previa, editar in-place) en vez de duplicar.
...
## Publicación inmediata para contenido de IA

El contenido sobre Inteligencia Artificial se desactualiza mucho más rápido que el resto del catálogo (un modelo, precio o benchmark puede quedar obsoleto de un día para otro) — por eso nunca sigue la cola normal de +2 días, sino que se publica lo antes posible.

**Qué cuenta como "tema IA"**: `categoriaPrincipal: "inteligencia-artificial"`, o cualquier infografía cuyo `temas[]` incluya `ia`, `agentes-ia`, `chatgpt`, `claude`, `prompt-engineering`, `machine-learning` o `deep-learning` — esto incluye explícitamente piezas sobre Claude, ChatGPT, Gemini o cualquier otro modelo/proveedor de IA, aunque el proveedor puntual no tenga su propia clave en `taxonomia.json`.

- **`fechaPublicacion` = el día de mañana** (fecha real de hoy + 1 día), no la fecha de la última entrada del catálogo + 2 días, y sin necesidad de que sea contenido de evento o de una actualización con aporte real de contenido — el solo hecho de tratar sobre IA activa esta regla.
- Aplica igual en `publicaciones.json` y en `paginaweb/publications/publicaciones.js` — cada uno usa el día de mañana en su propio formato de fecha (`publicaciones.json` sin offset; `publicaciones.js` con el offset horario que ya use ese archivo, ej. `-05:00`), pero es el mismo día calendario en ambos.
- Sigue aplicando la regla de nunca tener más de dos entradas con la misma fecha en el catálogo completo — si ya hay dos piezas de IA agendadas para "mañana", correr la nueva al día siguiente en vez de forzar una tercera.
- Esta regla es independiente de (y compatible con) el branding de evento: una infografía de IA para un evento puntual sigue usando la fecha/hora actual de todas formas, así que en la práctica coinciden salvo que "hoy" y "mañana" caigan en fechas distintas.
- No aplica retroactivamente a infografías de IA ya publicadas con una fecha futura por la cola normal — solo rige para las que se agreguen de aquí en adelante, salvo que el usuario pida explícitamente reagendar alguna ya existente.
