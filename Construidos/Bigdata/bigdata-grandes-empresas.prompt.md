...
## Contexto específico — Big Data en las Grandes Empresas

**Referencia**: `referencias/Bigdata/bigdata-grandes-empresas/Understanding_Big-Data_in_Big_Companies.pdf` — infografía de una sola página del International Institute for Analytics (Thomas H. Davenport, Jill Dyche) en colaboración con SAS Institute, mayo de 2013, analizada directamente.

**Importante — generalizar, no promocionar una marca**: la pieza original está copatrocinada por SAS. Se mantiene el contenido técnico y los casos reales citados, sin usar el logo de SAS como mensaje propio.

**Contenido real extraído** (sin inventar cifras):

1. **La pila técnica de Big Data** (stack real del documento): Storage → Platform Infrastructure → Data → Application (código, funciones y servicios) → Business View (modelos, vistas, cubos) → Presentation and Consumption, con "Data Movement" como flujo lateral que atraviesa todas las capas.
2. **Casos de éxito reales citados, con cifras verificables**: Macy's puede tasar precios de 73 millones de artículos en menos de 1 hora; UPS ahorró 8,4 millones de galones de combustible en 2011 optimizando rutas. Otras empresas citadas en el estudio: Caesars Entertainment (servicio en tiempo real para clientes top), UnitedHealthcare (mejor servicio al cliente), Bank of America (marketing personalizado mejorado), además de AIG, Carolinas Healthcare, Cigna, Dell, Discover, Fidelity, GE, Schneider National, Sears, T-Mobile y Verizon.
3. **Evolución de la analítica empresarial**: Analytics 1.0 (circa 1954) — pequeña, estructurada, foco interno; Analytics 2.0 (circa 2005) — más grande, foco más externo; Analytics 3.0 (2013) — masiva, no estructurada, más rápida, de alto potencial.
4. **Claves del éxito en Analytics 3.0** (cita textual del estudio): nuevas habilidades analíticas, liderazgo, estructuras organizacionales, tecnologías y arquitecturas de datos.

**Dirección visual**: paleta distinta a las otras 3 piezas del tema Bigdata (que usan teal/ámbar, navy/coral y navy/rojo institucional) — aquí grafito/acero + ámbar cálido, evocando un centro de datos empresarial. Composición: columna izquierda con la pila técnica como torre de bloques apilados (ecoando el gradiente de color del documento original); columna derecha superior con la línea de tiempo Analytics 1.0→2.0→3.0; columna derecha inferior con los dos casos de éxito con cifras reales (Macy's, UPS) como tarjetas destacadas. Cierre con las claves de éxito de Analytics 3.0.
