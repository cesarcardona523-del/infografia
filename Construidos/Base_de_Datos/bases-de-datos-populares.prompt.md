...
## Contexto específico — Bases de Datos Populares

**Referencia**: `referencias/Base_de_Datos/Base_de_datos_populares.pdf` (carrusel PDF de datdata.com, analizado directamente — no es una imagen). El PDF lista, en formato carrusel plano de logos + texto, 20 plataformas de bases de datos populares, sin ninguna categorización ni jerarquía visual entre ellas.

**Enriquecimiento** (no replicar el carrusel plano): se impone una categorización real que el PDF no tenía, agrupando las 16 plataformas más representativas en 4 familias funcionales:

1. **Relacionales (SQL)** — MySQL, PostgreSQL, Microsoft SQL Server, Oracle Database, SQLite, MariaDB.
2. **NoSQL** — MongoDB (documentos), Redis (memoria/caché), Cassandra (columnar distribuida), Couchbase (documentos + memoria).
3. **Gestionadas en la nube** — Amazon Aurora, Google Cloud Spanner, Azure SQL Database, Firebase Realtime Database.
4. **Especializadas** — Neo4j (grafos), Elasticsearch (búsqueda/análisis), Apache HBase (big data distribuido).

Las 4 restantes del PDF (Microsoft Access, IBM Db2, Amazon RDS como servicio genérico, Google Cloud Spanner ya cubierto) se omiten deliberadamente por espacio/relevancia editorial — no es necesario listar las 20 para comunicar el panorama.

**Dirección visual**: metáfora de "sala de servidores" (rack de discos apilados, ecoando el ícono literal de cilindros de base de datos que trae el PDF de referencia) — fondo oscuro tipo data center, 4 paneles verticales (uno por familia), cada panel con un color categórico fijo (no arcoíris arbitrario) y, dentro de cada panel, un pequeño punto de color con el tono de marca real de cada plataforma (MySQL azul/naranja, PostgreSQL azul, MongoDB verde, Redis rojo, etc.) como identificador secundario — igual que ya se hizo en `Arquitectura/arquitectura-google.html` con los 4 colores de Google. No reproducir logos oficiales — usar nombre + punto de color + ícono genérico de disco/base de datos.

**Elemento gráfico no textual**: el propio agrupamiento categórico en 4 paneles con color fijo funciona como el "gráfico" — adicionalmente, un pequeño indicador tipo "medidor" o barra de familiaridad/adopción no es necesario inventarlo (no hay cifras reales en el PDF); en su lugar, un ícono semántico distinto por familia (relacional=tabla, NoSQL=documento/hoja, cloud=nube, especializada=nodo de grafo) cumple el rol de riqueza visual sin inventar números.

**Paleta**: no hay marca única que anclar (son 16 marcas distintas) — se usa un fondo neutro oscuro tipo "centro de datos" con 4 acentos categóricos fijos (uno por familia) en vez del verde de la casa, ya que este tema es multi-vendor por naturaleza.
