...
## Contexto específico — BI vs BA

**Referencia**: `referencias/BI/bi-vs-ba/BI_BA.jpeg` (imagen ya terminada, de Juan Carlos Figueroa) — split-screen: BI (Business Intelligence) representado con un espejo retrovisor ("mirando al pasado"), BA (Business Analytics) representado con una brújula ("anticipando el futuro"), cada lado con 4 capacidades.

**Enriquecimiento** (no replicar 1:1, complementar): la referencia ya trae una metáfora sólida (retrovisor vs. brújula) — se conserva esa metáfora central pero se agrega una **línea de tiempo explícita** (Pasado → Presente → Futuro) que la referencia no tenía, para reforzar visualmente por qué BI mira hacia atrás y BA hacia adelante, y un cierre que aclare que no son alternativas excluyentes sino complementarias (BA se construye sobre los datos que BI ya organizó).

**Contenido por lado** (igual al de la referencia):
- **BI — mirando al pasado**: datos históricos y presentes, reporting & dashboards, análisis descriptivo (¿qué pasó?), optimización operativa.
- **BA — anticipando el futuro**: modelado predictivo & ML, predicción de tendencias, análisis prescriptivo (¿qué debemos hacer?), descubrimiento de oportunidades estratégicas.

**Dirección visual**: split-screen vertical, mitad izquierda en azul-acero frío (pasado, estable, retrospectivo) y mitad derecha en verde-esmeralda (futuro, proyección, crecimiento) — coherente con los colores ya usados en la referencia, pero con paleta propia (no una copia exacta de sus tonos). Ícono grande de espejo retrovisor a la izquierda, de brújula a la derecha. Línea de tiempo horizontal en la parte superior, cruzando ambas mitades, con marcador en el centro señalando "hoy". Cierre inferior: nota de que BI y BA se complementan, no compiten.

**Elemento gráfico no textual**: la línea de tiempo horizontal Pasado/Presente/Futuro con marcador central, más los dos íconos centrales (espejo, brújula) como ilustraciones vectoriales con propósito semántico exacto.
